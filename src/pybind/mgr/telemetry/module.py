...
# ALL_CHANNELS = ['basic', 'ident', 'crash', 'device', 'perf', 'dashboard']
...
#         Option(name='channel_dashboard',
#                type='bool',
#                default=True,
#                desc='Share Dashboard user adoption and page visit metrics'),
#     ]
...
#     def get_dashboard_telemetry(self) -> dict:
#         """Public wrapper for dashboard telemetry - callable via remote()"""
#         return self._get_dashboard_telemetry()

#     def _get_dashboard_telemetry(self) -> dict:
#         self.log.info('[Dashboard Telemetry] _get_dashboard_telemetry() called')
#         try:
#             mgr_map = self.get('mgr_map')
#             modules = mgr_map.get('modules', [])

#             dashboard_enabled = 'dashboard' in modules
#             prometheus_enabled = 'prometheus' in modules

#             service_map = self.get('service_map')
#             services = service_map.get('services', {})

#             alertmanager_enabled = 'alertmanager' in services
#             grafana_enabled = 'grafana' in services

#             if not dashboard_enabled:
#                 status = 1
#                 health = 'not_enabled'
#             elif dashboard_enabled and prometheus_enabled:
#                 status = 0
#                 health = 'healthy'
#             else:
#                 status = 2
...
#             # Read page visit events from config-key store
#             events = {}
#             total_visits = 0
#             try:
#                 # Get total page visits
#                 total_key = 'dashboard/telemetry/total_page_visits'
#                 total_val = self.get_store(total_key)
#                 if total_val:
#                     total_visits = int(total_val)
#                     self.log.info(f'[Dashboard Telemetry] Total visits: {total_visits}')
                
#                 # Get individual page visit counts
#                 # We need to list all keys with prefix 'dashboard/telemetry/'
#                 # Since we don't have a list_keys method, we'll try known event names
#                 known_events = [
#                     'rgw.bucket_list',
#                     'pools.list',
#                     'osds.list',
...
#                     key = f'dashboard/telemetry/{event_name}'
#                     val = self.get_store(key)
#                     if val:
#                         events[event_name] = int(val)
#                         self.log.info(f'[Dashboard Telemetry] Event {event_name}: {val}')
                
#             except Exception as e:
#                 self.log.warning('[Dashboard Telemetry] Failed to read events: %s', e)

#             # Detect deployment type
#             deployment_type = 'unknown'
#             try:
#                 # Check for cephadm orchestrator
#                 orch_status = self.get('orchestrator_status')
#                 if orch_status:
#                     backend = orch_status.get('backend', '')
#                     if 'cephadm' in backend.lower():
#                         deployment_type = 'cephadm'
#                     elif 'rook' in backend.lower():
#                         deployment_type = 'rook'
#                     else:
#                         deployment_type = 'manual'
#                 else:
#                     deployment_type = 'manual'
#                 self.log.info(f'[Dashboard Telemetry] Deployment type: {deployment_type}')
#             except Exception as e:
#                 self.log.warning('[Dashboard Telemetry] Failed to detect deployment type: %s', e)
...
#             try:
#                 # Check for RBD (block storage)
#                 osd_map = self.get('osd_map')
#                 if osd_map:
#                     pools = osd_map.get('pools', [])
#                     for pool in pools:
#                         app_metadata = pool.get('application_metadata', {})
#                         if 'rbd' in app_metadata:
#                             block_enabled = True
#                             block_pools += 1
#                     self.log.info(f'[Dashboard Telemetry] Block storage: enabled={block_enabled}, pools={block_pools}')
#             except Exception as e:
#                 self.log.warning('[Dashboard Telemetry] Failed to detect block storage: %s', e)

#             try:
#                 # Check for CephFS (file storage)
#                 fs_map = self.get('fs_map')
#                 if fs_map:
#                     filesystems = fs_map.get('filesystems', [])
#                     file_count = len(filesystems)
#                     file_enabled = file_count > 0
#                     self.log.info(f'[Dashboard Telemetry] File storage: enabled={file_enabled}, count={file_count}')
#             except Exception as e:
#                 self.log.warning('[Dashboard Telemetry] Failed to detect file storage: %s', e)

#             try:
#                 # Check for RGW (object storage)
#                 service_map = self.get('service_map')
#                 if service_map:
#                     services = service_map.get('services', {})
#                     rgw_service = services.get('rgw', {})
#                     if rgw_service:
#                         daemons = rgw_service.get('daemons', {})
#                         object_daemons = len(daemons)
#                         object_enabled = object_daemons > 0
                        
#                         # Try to get zone count (this is approximate)
#                         # In a real implementation, we'd query RGW admin API
#                         object_zones = 1 if object_enabled else 0
                        
#                         self.log.info(f'[Dashboard Telemetry] Object storage: enabled={object_enabled}, daemons={object_daemons}, zones={object_zones}')
#             except Exception as e:
#                 self.log.warning('[Dashboard Telemetry] Failed to detect object storage: %s', e)
...
#                     'object_daemons': object_daemons,
#                     'object_zones': object_zones
#                 },
#                 'events': events
#             }

#             self.log.info(f'[Dashboard Telemetry] Returning data with {len(events)} events, total visits: {total_visits}')
#             return dashboard_data

#         except Exception as e:
#             self.log.warning('[Dashboard Telemetry] Failed: %s', e)
#             return {
#                 'version': 1,
#                 'adoption': {
...
#             report['dashboard'] = self._get_dashboard_telemetry()
        ...
        try:
            ...
            report['dashboard'] = {
                'page_visits': {
                    page: int(
                        _read_key(
                            'mgr/dashboard/ui_metrics/page_visits/' + page
                        ) or 0
                    )
                ...
                'sessions': {
                    'login_count': int(
                        _read_key(
                            'mgr/dashboard/ui_metrics/sessions/login_count'
                        ) or 0
                    ),
                    'last_login_epoch': int(
                        _read_key(
                            'mgr/dashboard/ui_metrics/sessions/last_login_epoch'
                        ) or 0
                    ),
                },
                ...
                'user_personas': _json.loads(
                    _read_key(
                        'mgr/dashboard/ui_metrics/user_personas'
                    ) or '{}'
                ),
            }
